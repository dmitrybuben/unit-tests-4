...
class PaymentFormTest {
     ...
     * Создайте класс `CreditCard` с методами `getCardNumber()`, `getCardHolder()`, `getExpiryDate()`, `getCvv()`,
    ...
    void testPayMethodInPaymentForm() {
        ...
        form.pay(amount);
        /*т.к. метод charge - void, то мок конструкция when...then не вызывается, а только verify
        в verify передается 1 моковый объект - 1 verify - 1 тест и проверяется как он себя ведет
        times - сколько раз вызывается метод*/
